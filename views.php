...
class Advanced_Image_Grabber_views{
	...
	static public function adminSettingsPage(){
		wp_enqueue_style('advanced_image_grabber_style', plugin_dir_url(__FILE__).'css/style.css');
		wp_enqueue_style('advanced_image_grabber_picker_style', plugin_dir_url(__FILE__).'css/image-picker.css');
		wp_enqueue_script('advanced_image_grabber_picker_script', plugin_dir_url(__FILE__).'js/image-picker.min.js');
		?>
        ...
        <script language="javascript"> //Ajax call on button click
			jQuery('#aig_grab_images').click(function(){
				...
				if(urls){
				jQuery('.aig_img_listbox').html('<img src="<?php echo plugin_dir_url(__FILE__).'images/loading.gif'; ?>" style="width:150px;height:150px;" />');
   				jQuery.ajax({
        		url: ajaxurl,
        		data: {
